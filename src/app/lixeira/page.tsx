...
export default async function LixeiraPage() {
  ...
  return (
    <AppShell eyebrow="Administração" title="Lixeira" user={user} search={false}>
      ...
      >
        <p className="mb-4 rounded-lg border border-[#d9e1dd] bg-[#f8faf9] px-3 py-2.5 text-sm leading-6 text-[#66756d]">
        ...
        {itens.length === 0 ? (
          <div className="rounded-lg border border-dashed border-[#c7d3ce] bg-[#f8faf9] p-8 text-center">
            <Undo2 size={24} className="mx-auto text-[#05605e]" aria-hidden="true" />
            <h3 className="mt-2 font-semibold">Lixeira vazia</h3>
            <p className="mt-1 text-sm text-[#66756d]">Nenhum cadastro foi excluído.</p>
          </div>
        ) : (
          <ul className="divide-y divide-[#eef2ef]">
            {itens.map((item) => (
              <li key={`${item.tipo}-${item.id}`} className="flex flex-wrap items-center gap-x-3 gap-y-2 py-3 first:pt-0 last:pb-0">
                <span className="rounded-md border border-[#d9e1dd] bg-[#f8faf9] px-2 py-0.5 text-xs font-semibold text-[#405047]">
                  {rotuloTipo(item.tipo)}
                </span>
                <span className="font-semibold">{item.nome}</span>
                {item.detalhe ? <span className="text-sm text-[#8a9890]">{item.detalhe}</span> : null}

                <span className="ml-auto text-xs text-[#8a9890]">
                  {(() => {
                ...
                <div className="flex items-center gap-2">
                  <ToastForm action={restoreAction}>
                    ...
                    <button className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#bfe0d9] bg-[#e8f6f3] px-3 text-sm font-semibold text-[#05605e] transition hover:bg-[#d9efe9]">
                  ...
                  {podeApagar ? (
                    <ToastForm action={purgeAction} confirm={textoApagarDeVez(item.tipo, item.nome)}>
                      ...
                      <input type="hidden" name="tipo" value={item.tipo} />
                      <button
                        className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#f1c0c9] bg-white px-3 text-sm font-semibold text-[#9f2f42] transition hover:bg-[#fff0f2]"
