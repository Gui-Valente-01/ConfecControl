  ...
  return (
    <section className="grid gap-6 xl:grid-cols-[1fr_360px]">
      ...
      <SectionCard
        ...
        action={<div className="rounded-lg border border-line bg-tint px-3 py-2 text-sm font-semibold text-body">{partners.length} empresa(s)</div>}
      >
        {partners.length === 0 ? (
          <div className="rounded-lg border border-dashed border-line-strong bg-canvas p-8 text-center">
            <Handshake className="mx-auto text-primary" size={28} aria-hidden="true" />
            <h3 className="mt-3 font-semibold">Nenhuma terceirizada cadastrada</h3>
            <p className="mt-2 text-sm text-muted">Cadastre empresas que fazem serviços externos (bordado, estampa, lavanderia...).</p>
          </div>
        ) : (
          <div className="grid gap-4 lg:grid-cols-2">
            {partners.map((partner) => (
            <article key={partner.id} className="rounded-lg border border-line bg-surface p-4 shadow-sm transition hover:border-line-strong">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-sm text-muted">{partner.service || "Serviço não informado"}</p>
                    </div>
                  </div>
                ...
                <div className="mt-3 space-y-1 text-sm text-body">
                  ...
                  {partner.notes ? <p className="text-muted">{partner.notes}</p> : null}
                  <p className="text-xs text-soft">{partner.orderCount} pedido(s) vinculado(s)</p>
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <ToastForm action={togglePartnerActiveAction}>
                    ...
                    <button className="inline-flex h-9 items-center gap-1 rounded-lg border border-line-strong bg-surface px-3 text-xs font-semibold text-body transition hover:bg-canvas">
      ...
      <SectionCard eyebrow="Nova terceirizada" title="Cadastrar empresa">
        <form ref={formRef} className="space-y-3" action={formAction}>
          ...
          <label className="block">
            <span className="text-sm font-medium text-body">Serviço prestado</span>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              <span className="text-sm font-medium text-body">Telefone</span>
          ...
          <label className="block">
            ...
            <textarea name="notes" className="mt-1 min-h-20 w-full rounded-lg border border-line-strong px-3 py-2 text-sm outline-none ring-primary/20 transition focus:border-primary focus:ring-4" placeholder="Prazo médio, valores, etc." />
          </label>
